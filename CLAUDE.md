...
Project: `LiveSubtitleTranslator.xcodeproj`. Swift 5.0, macOS deployment target **26.5**, hardened runtime + App Sandbox, and shared project signing metadata with `DEVELOPMENT_TEAM` intentionally blank. Select a local signing team in Xcode if required, but do not commit personal team IDs. `LSUIElement = YES` (menu-bar agent; no Dock icon). `Info.plist` is generated by Xcode (`GENERATE_INFOPLIST_FILE = YES`); add new keys via `INFOPLIST_KEY_*` build settings rather than committing a plist file. `NSAudioCaptureUsageDescription` is already set.
